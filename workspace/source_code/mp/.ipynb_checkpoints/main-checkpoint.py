...
train_all_set = datasets.MNIST('../data/mnist_data', download=True, train=True,
...
def train(args):
    model = MyNet()
    model.train()
    trainset= datasets.MNIST('../data/mnist_data', download=True, train=True,
               transform = transforms.Compose([transforms.ToTensor(),
               transforms.Normalize((0.1307,),(0.3081,))]))
...
def test(args, model):
    model.eval()
    testset = datasets.MNIST('../data/mnist_data', download=True, train=True,
               transform = transforms.Compose([transforms.ToTensor(),
               transforms.Normalize((0.1307,),(0.3081,))]))
